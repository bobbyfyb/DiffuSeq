from transformers import AutoConfig, LongT5EncoderModel
import torch
import numpy as np
...
class TransformerNetModel(nn.Module):
    ...
    def __init__(
        ...
        dropout=0,
        config=None,
        config_name='google/long-t5-local-base',
        vocab_size=None,
        init_pretrained='no',
        logits_mode=1,
        ...
        if init_pretrained == 'longt5':
            print('initializing from pretrained longt5...')
            print(config)
            temp_longt5 = LongT5EncoderModel.from_pretrained(config_name, config=config)

            self.word_embedding = temp_longt5.shared
            with torch.no_grad():
                self.lm_head.weight = self.word_embedding.weight

            self.input_transformers = temp_longt5.encoder
            self.register_buffer("position_ids", torch.arange(config.max_position_embeddings).expand((1, -1)))
            self.position_embeddings = temp_longt5.embeddings.position_embeddings
            self.LayerNorm = temp_longt5.embeddings.LayerNorm

            del temp_longt5.embeddings
            del temp_longt5.pooler

        elif init_pretrained == 'no':
            self.input_transformers = LongT5EncoderModel(config)
